...
def parse_args():
    ...
    parser.add_argument(
        '--input', type=str, 
        default='/aivanas/raw/surveillance/action/violence/action_recognition/data/RWF-2000/train/Fight/0_DzLlklZa0_3.avi', help='Image/Video file')  # 입력 파일 경로
    parser.add_argument(
        '--show',
        action='store_true',
        default=True,
...
def main():
    ...
    if os.path.isdir(args.input):
        ...
        for video_path in video_files:
            ...
            args.input = video_path  # 각 파일을 input으로 설정
            # 아래 기존 단일 파일 처리 코드 재사용
            # (output_file, pred_save_path 등은 args.input 기준으로 생성됨)
            # 기존 main()의 단일 파일 처리 코드 복사
            output_file = None
            if args.output_root:
            ...
            if input_type == 'image':
                ...
                print(f"file {os.path.basename(args.input)} has invalid format. 건너뜀.")
            if args.save_predictions:
        ...
        if args.input == 'webcam':
            input_type = 'webcam'  # 입력이 웹캠이면 webcam
        else:
            input_type = mimetypes.guess_type(args.input)[0].split('/')[0]  # 파일 타입 추론

        if input_type == 'image':
            # 이미지 추론
            pred_instances = process_one_image(
                args, args.input, model, visualizer, show_interval=0)  # 한 장 추론

            if args.save_predictions:
                pred_instances_list = split_instances(pred_instances)  # 인스턴스 분리

            if output_file:
                img_vis = visualizer.get_image()  # 시각화 이미지 얻기
                mmcv.imwrite(mmcv.rgb2bgr(img_vis), output_file)  # 파일로 저장
            ...
            if args.input == 'webcam':
                ...
                cap = cv2.VideoCapture(args.input)  # 비디오 파일 캡처 객체 생성

            video_writer = None  # 비디오 저장 객체 초기화
            pred_instances_list = []  # 예측 인스턴스 리스트 초기화
            ...
            while cap.isOpened():  # 비디오가 열려있는 동안 반복
                ...
                # 비디오 저장
                if output_file:
                    frame_vis = visualizer.get_image()  # 시각화 프레임 얻기

                    if video_writer is None:
                        fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 코덱 설정
                        # heatmap 유무에 따라 이미지 크기가 달라질 수 있음
                        video_writer = cv2.VideoWriter(
                            output_file,
                            fourcc,
                            25,  # 저장 fps
                            (frame_vis.shape[1], frame_vis.shape[0]))  # 프레임 크기

                    video_writer.write(mmcv.rgb2bgr(frame_vis))  # 프레임 저장
                ...
                if args.show:
                    ...
                    time.sleep(args.show_interval)  # 프레임 간 대기

            if video_writer:
                video_writer.release()  # 비디오 저장 종료
            ...
            args.save_predictions = False
            raise ValueError(
                f'file {os.path.basename(args.input)} has invalid format.')  # 지원하지 않는 입력 형식 예외
        ...
        if args.save_predictions:
            with open(args.pred_save_path, 'w') as f:
                json.dump(
                    ...
                    f,
                    indent='\t')  # 예측값 json 저장
            print(f'predictions have been saved at {args.pred_save_path}')  # 저장 경로 출력
        ...
        if output_file:
            ...
            print_log(
                f'the output {input_type} has been saved at {output_file}',
                logger='current',
                level=logging.INFO)  # 결과 저장 경로 로그 출력
