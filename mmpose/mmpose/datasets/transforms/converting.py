...
class KeypointConverter(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # print(f"[KeypointConverter] transform init results: {results['keypoints'].shape}")
        ...
        if flip_indices is not None:
            # CrowdPose → COCO 변환 시 flip_indices도 변환
            if self.num_keypoints == 17:  # COCO 형식으로 변환하는 경우
                # COCO 17개 키포인트의 좌우 대칭 관계
        ...
        # print(f"[KeypointConverter] flip_indices results: {results['flip_indices']}")
        # print(f"[KeypointConverter] transform output results: {results['keypoints'].shape}")
        return results
