...
class RandomFlip(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if flip_dir is None:
            ...
            # print(f"[RandomFlip-1] : {results['keypoints'].shape}")
            results['flip'] = True
            ...
            if results.get('bbox', None) is not None:
                # print(f"[RandomFlip-bbox] : {results['bbox'].shape}")
            ...
            if results.get('bbox_center', None) is not None:
                # print(f"[RandomFlip-bbox_center] : {results['bbox_center'].shape}")
        ...
        # print(f"[RandomFlip_after] transform output results: {results['keypoints'].shape}")   
        return results
...
class GenerateTarget(BaseTransform):
    ...
    def transform(self, results: Dict) -> Optional[dict]:
        ...
        # print(f"[GenerateTarget] transform output results: {results['keypoints'].shape}")
        return results
...
class YOLOXHSVRandomAug(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['img'] = img
        # print(f"[YOLOXHSVRandomAug] transform output results: {results['keypoints'].shape}")
        return results
...
class FilterAnnotations(BaseTransform):
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        # print(f"[FilterAnnotations] transform output results: {results['keypoints'].shape}")
        return results
