...
class YOLOXPoseModeSwitchHook(Hook):
    ...
    def __init__(self,
        ...
        print('[YOLOXPoseModeSwitchHook] 🔄 Initializing.../workspace')
    ...
    def _modify_dataloader(self, runner: Runner):
        ...
        print('[YOLOXPoseModeSwitchHook] 🔄 Modifying dataloader...')
        print(f'[YOLOXPoseModeSwitchHook] New Dataset: {self.new_train_dataset}')
        print(f'[YOLOXPoseModeSwitchHook] New Pipeline: {self.new_train_pipeline}')
    ...
    def before_train_epoch(self, runner: Runner):
        ...
        # 디버깅을 위한 로그 출력 추가
        print(f'[YOLOXPoseModeSwitchHook-git] Current epoch: {epoch}, Max epochs: {runner.max_epochs}, Num last epochs: {self.num_last_epochs}')
        print(f'[YOLOXPoseModeSwitchHook-git] Switch condition: {epoch + 1} == {runner.max_epochs - self.num_last_epochs} = {epoch + 1 == runner.max_epochs - self.num_last_epochs}')

        if epoch + 1 == runner.max_epochs - self.num_last_epochs:
            print(f'[YOLOXPoseModeSwitchHook-git] 🎯 SWITCHING TO CUSTOM DATASET AND PIPELINE at epoch {epoch + 1}!')
            self._modify_dataloader(runner)
            print('Added additional reg loss now!')
            model.head.use_aux_loss = True
        else:
            print(f'[YOLOXPoseModeSwitchHook-git] ⏳ Waiting for switch condition... (epoch {epoch + 1})')
