...
사용법:
  ...
  # Step2 설정을 지정하여 stage 3 실행
  python separated_pose_pipeline.py --stage 3 --step2-config clip100_stride10_thresh0.6_0.1_quality0.3
...
from error_logging.error_logger import ProcessingErrorLogger
...
class SeparatedPosePipeline:
    ...
    def run_stage3_unification(self, force_reprocess=False, step2_config=None):
        """3단계: 통합 PKL 생성"""
        print("=" * 60)
        print("STAGE 3: Dataset Unification")
        print("=" * 60)
        
        # 2단계 결과 수집 (step2 설정 선택 가능)
        tracking_results = self._collect_tracking_results(step2_config)
        
        if not tracking_results:
            print("No tracking results found from stage 2. Please run stage 2 first.")
            if step2_config:
                print(f"Specified step2 config '{step2_config}' not found.")
                self._show_available_step2_configs()
            return
    ...
    def _apply_tracking_to_video(self, poses_file: str):
        ...
        from core.annotation_generator import create_enhanced_annotation
    ...
    def _collect_tracking_results(self, step2_config=None) -> List[str]:
        """2단계 트래킹 결과 수집"""
        tracking_files = []
        dataset_name = os.path.basename(self.config.input_dir.rstrip('/\\'))
        step2_base_dir = os.path.join(self.config.output_dir, dataset_name, "step2_tracking")
        
        # step2_config가 지정된 경우 해당 설정 폴더만 사용
        if step2_config:
            target_dir = os.path.join(step2_base_dir, step2_config)
            if not os.path.exists(target_dir):
                print(f"Error: Step2 config directory not found: {target_dir}")
                return []
            
            print(f"Using step2 config: {step2_config}")
            for category in ['Fight', 'NonFight']:
            ...
            # step2_config가 지정되지 않은 경우 사용 가능한 설정들을 확인하고 선택
            available_configs = self._get_available_step2_configs()
            
            if len(available_configs) == 0:
                print("No step2 configurations found")
                return []
            elif len(available_configs) == 1:
                # 하나만 있으면 자동으로 선택
                selected_config = available_configs[0]
                print(f"Auto-selecting the only available step2 config: {selected_config}")
                target_dir = os.path.join(step2_base_dir, selected_config)
            else:
                # 여러 개가 있으면 사용자에게 선택하도록 안내
                print(f"Multiple step2 configurations found:")
                for i, config in enumerate(available_configs, 1):
                    print(f"  {i}. {config}")
                print(f"\nPlease specify which step2 config to use with --step2-config option:")
                print(f"  Example: --step2-config {available_configs[0]}")
                return []
    ...
    def _get_available_step2_configs(self) -> List[str]:
        """사용 가능한 step2 설정들을 반환"""
        dataset_name = os.path.basename(self.config.input_dir.rstrip('/\\'))
        step2_base_dir = os.path.join(self.config.output_dir, dataset_name, "step2_tracking")
        ...
        configs = []
        for item in os.listdir(step2_base_dir):
            item_path = os.path.join(step2_base_dir, item)
            if os.path.isdir(item_path):
                # Fight나 NonFight 폴더가 있는지 확인
                if (os.path.exists(os.path.join(item_path, 'Fight')) or 
                    os.path.exists(os.path.join(item_path, 'NonFight'))):
                    configs.append(item)
        
        return sorted(configs)
    
    def _show_available_step2_configs(self):
        """사용 가능한 step2 설정들을 표시"""
        available_configs = self._get_available_step2_configs()
        if available_configs:
            print("Available step2 configurations:")
            for i, config in enumerate(available_configs, 1):
                print(f"  {i}. {config}")
            print(f"\nUse --step2-config option to specify which one to use:")
            print(f"  Example: --step2-config {available_configs[0]}")
        else:
            print("No step2 configurations found. Please run stage 2 first.")
...
def main():
    ...
    parser.add_argument('--step2-config', type=str,
                       help='Specific step2 configuration folder name to use for stage 3 (e.g., clip100_stride10_thresh0.6_0.1_quality0.3)')
    ...
    try:
        ...
        if args.stage in ['3', 'all']:
            ...
            step2_config = getattr(args, 'step2_config', None)  # --step2-config의 값 가져오기 (하이픈이 언더스코어로 변환됨)
            pipeline.run_stage3_unification(force_reprocess=args.force, step2_config=step2_config)
